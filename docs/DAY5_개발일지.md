# DAY 5 개발일지 - 피드 및 검색 기능 구현

**날짜**: 2025년 11월 29일  
**목표**: 콘텐츠 탐색 및 검색 기능 구현

---

## 📋 오늘의 목표

1. 피드 페이지 구현 (팔로잉 피드, 탐색 피드)
2. 피드 정렬 기능 (최신순, 인기순)
3. 검색 기능 구현 (마커 검색, 사용자 검색)
4. 지도 페이지 검색 기능
5. 무한 스크롤 구현 (Intersection Observer 활용)
6. 인기 마커 조회 기능
7. 주변 마커 조회 기능

---

## ✅ 완료 작업

### 1. 피드 페이지 구현

#### 페이지 구성
- 탭 전환 (팔로잉 / 탐색)
- 정렬 옵션 (NEW / HOT)
- 마커 카드 목록
- 무한 스크롤

#### 팔로잉 피드
- **API**: `GET /api/feed`
- 팔로우한 사용자들의 마커만 표시
- 로그인 필수
- 최신순 정렬

#### 탐색 피드
- **API**: `GET /api/feed/explore`
- 모든 사용자의 마커 표시
- 로그인 불필요
- 정렬 옵션 (최신순, 인기순)

#### 마커 카드 구성
- 프로필 이미지 및 닉네임
- 마커 이미지
- 3줄 텍스트
- 좋아요 수, 댓글 수
- 작성 시간
- 좋아요/북마크 버튼
- 지도 보기 버튼

### 2. 피드 정렬 기능

#### 정렬 옵션
- **NEW (최신순)**: CREATED_AT 기준 내림차순
- **HOT (인기순)**: 좋아요 수 + 댓글 수 기준 내림차순

#### 구현 방법
- 서버 사이드 정렬
- 정렬 옵션을 쿼리 파라미터로 전달
- 정렬 변경 시 피드 새로고침

#### 코드 예시
```sql
-- 최신순
ORDER BY CREATED_AT DESC

-- 인기순
ORDER BY (LIKE_COUNT + COMMENT_COUNT) DESC, CREATED_AT DESC
```

### 3. 검색 기능 구현

#### 통합 검색
- **API**: `GET /api/search`
- 마커와 사용자를 동시에 검색
- 검색어 하이라이트

#### 마커 검색
- **API**: `GET /api/search/markers`
- 마커 텍스트 (line1, line2, line3) 검색
- 카테고리 필터링
- 위치 기반 검색 (추후 구현)

#### 사용자 검색
- **API**: `GET /api/search/users`
- 사용자 ID, 닉네임 검색
- 프로필 이미지 표시

#### 검색 결과 표시
- 마커 결과와 사용자 결과 분리
- 결과 클릭 시 해당 페이지로 이동
- 검색어 하이라이트

### 4. 지도 페이지 검색 기능

#### 검색 입력 컴포넌트
- `MapSearchInput` 컴포넌트 생성
- 자동완성 기능
- 디바운싱 적용 (300ms)

#### 검색 결과 표시
- 드롭다운 목록
- 마커 결과 클릭 시 지도 이동
- 사용자 결과 클릭 시 프로필 페이지 이동

#### 지도 연동
- 검색 결과 마커 하이라이트
- 선택된 마커로 지도 이동
- 마커 클러스터 업데이트

### 5. 무한 스크롤 구현

#### Intersection Observer 활용
- 하단에 감시 영역 배치
- 감시 영역이 보이면 다음 페이지 로드
- 로딩 상태 표시

#### 구현 세부사항
- `useRef`로 감시 영역 참조
- `useCallback`으로 콜백 함수 메모이제이션
- 페이지 번호 관리
- 더 이상 데이터가 없을 때 처리

#### 코드 구조
```javascript
const loadMoreRef = useRef();
const [page, setPage] = useState(1);
const [hasMore, setHasMore] = useState(true);

useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && hasMore && !loading) {
            loadFeed(page + 1);
        }
    }, {
        root: null,
        rootMargin: '100px',
        threshold: 0
    });
    
    if (loadMoreRef.current) {
        observer.observe(loadMoreRef.current);
    }
    
    return () => observer.disconnect();
}, [hasMore, loading, page, loadFeed]);
```

### 6. 인기 마커 조회 기능

#### API 구현
- `GET /api/markers/popular`
- 좋아요 수 + 댓글 수 기준 정렬
- 기간 필터링 (오늘, 이번 주, 이번 달)

#### 활용
- 탐색 피드의 HOT 정렬
- 메인 페이지 추천 마커 (추후 구현)

### 7. 주변 마커 조회 기능

#### API 구현
- `GET /api/markers/nearby`
- 현재 위치 기준 반경 내 마커 조회
- 거리 계산 (Haversine formula)

#### 구현 세부사항
- 위도/경도 파라미터
- 반경 설정 (기본 5km)
- 거리순 정렬

#### 코드 예시
```sql
SELECT *,
    (6371 * acos(
        cos(radians(?)) * cos(radians(LATITUDE)) *
        cos(radians(LONGITUDE) - radians(?)) +
        sin(radians(?)) * sin(radians(LATITUDE))
    )) AS distance
FROM LM_MARKERS
HAVING distance < ?
ORDER BY distance
LIMIT ?;
```

---

## 🐛 주요 이슈 및 해결

### 이슈 1: 무한 스크롤 성능 최적화
**문제**:  
- 과도한 리렌더링
- 메모리 사용량 증가
- 스크롤 성능 저하

**해결 과정**:
1. `useCallback`으로 함수 메모이제이션
2. `useMemo`로 계산 결과 메모이제이션
3. 불필요한 리렌더링 방지
4. 페이지 크기 조정 (10개씩)

**코드**:
```javascript
const loadFeed = useCallback(async (pageNum, reset = false) => {
    if (loading) return;
    
    setLoading(true);
    try {
        const data = await getFeed(token, pageNum, 10);
        
        if (reset) {
            setMarkers(data.markers);
        } else {
            setMarkers(prev => [...prev, ...data.markers]);
        }
        
        setHasMore(data.markers.length >= 10);
    } catch (error) {
        console.error('피드 로드 실패:', error);
    } finally {
        setLoading(false);
    }
}, [token, tab, sort, isAuthenticated]);
```

### 이슈 2: 검색 성능 개선
**문제**:  
- 입력할 때마다 API 요청 발생
- 서버 부하 증가
- 불필요한 요청

**해결 과정**:
1. 디바운싱 적용 (300ms)
2. 최소 검색어 길이 제한 (2자)
3. 서버 사이드 필터링

**코드**:
```javascript
const useDebounce = (value, delay) => {
    const [debouncedValue, setDebouncedValue] = useState(value);
    
    useEffect(() => {
        const handler = setTimeout(() => {
            setDebouncedValue(value);
        }, delay);
        
        return () => clearTimeout(handler);
    }, [value, delay]);
    
    return debouncedValue;
};

const debouncedSearchTerm = useDebounce(searchTerm, 300);
```

### 이슈 3: 검색 결과 하이라이트
**문제**:  
- 검색어가 결과에 하이라이트되지 않음
- 사용자가 검색 결과를 찾기 어려움

**해결 과정**:
1. 검색어를 정규식으로 변환
2. 결과 텍스트에서 검색어 찾기
3. `<mark>` 태그로 하이라이트

**코드**:
```javascript
const highlightText = (text, searchTerm) => {
    if (!searchTerm) return text;
    
    const regex = new RegExp(`(${searchTerm})`, 'gi');
    return text.replace(regex, '<mark>$1</mark>');
};
```

### 이슈 4: 피드 탭 전환 시 상태 초기화
**문제**:  
- 탭 전환 시 이전 데이터가 남아있음
- 정렬 변경 시 데이터가 섞임

**해결 과정**:
1. 탭/정렬 변경 시 상태 초기화
2. 페이지 번호 리셋
3. 새로 데이터 로드

**코드**:
```javascript
useEffect(() => {
    setMarkers([]);
    setPage(1);
    setHasMore(true);
    loadFeed(1, true);
}, [tab, sort, isAuthenticated, loadFeed]);
```

---

## 📝 코드 구조

### Frontend 구조
```
frontend/src/
├── pages/
│   ├── FeedPage.jsx         # 피드 페이지
│   └── MapPage.jsx          # 지도 페이지 (검색 기능 추가)
├── components/
│   └── ui/
│       └── MapSearchInput.jsx # 지도 검색 입력
└── hooks/
    └── useMarkers.js        # 마커 데이터 관리 훅
```

### Backend 구조
```
backend/src/
├── controllers/
│   ├── feedController.js    # 피드 컨트롤러
│   └── searchController.js # 검색 컨트롤러
└── routes/
    ├── feedRoutes.js        # 피드 라우터
    └── searchRoutes.js      # 검색 라우터
```

---

## 💡 배운 점

1. **무한 스크롤 구현**
   - Intersection Observer API 활용
   - 성능 최적화 방법
   - 상태 관리

2. **검색 기능**
   - 디바운싱을 통한 성능 개선
   - 서버 사이드 필터링
   - 검색 결과 하이라이트

3. **피드 시스템**
   - 팔로잉 피드 vs 탐색 피드
   - 정렬 옵션 구현
   - 탭 전환 시 상태 관리

4. **성능 최적화**
   - useCallback 및 useMemo 활용
   - 불필요한 리렌더링 방지
   - 페이지네이션

---

## 📊 작업 통계

- **작성한 파일 수**: 약 6개
- **코드 라인 수**: 약 2,000줄
- **구현한 API 엔드포인트**: 5개
- **생성한 컴포넌트**: 2개
- **생성한 훅**: 1개

---

## 🎯 내일 계획

1. Socket.io 서버 설정
2. Socket.io 클라이언트 통합
3. 일대일 채팅 기능 구현
4. 채팅방 목록 조회
5. 실시간 메시지 전송 및 수신

---

## 📸 참고 자료

- [Intersection Observer API](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API)
- [React Hooks 최적화](https://react.dev/reference/react/useCallback)

---
